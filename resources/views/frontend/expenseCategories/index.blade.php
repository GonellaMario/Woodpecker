                                    ...
                                    <tr data-entry-id="{{ $expenseCategory->id }}">
                                        ...
                                        <td>
                                            @can('expense_category_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.expense-categories.show', $expenseCategory->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('expense_category_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.expense-categories.edit', $expenseCategory->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('expense_category_delete')
                                                <form action="{{ route('frontend.expense-categories.destroy', $expenseCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
