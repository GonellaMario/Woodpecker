        ...
        <div class="col-md-12">
            @can('time_work_type_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.time-work-types.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $timeWorkType->id }}">
                                        ...
                                        <td>
                                            @can('time_work_type_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.time-work-types.show', $timeWorkType->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('time_work_type_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.time-work-types.edit', $timeWorkType->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('time_work_type_delete')
                                                <form action="{{ route('frontend.time-work-types.destroy', $timeWorkType->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('time_work_type_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.time-work-types.massDestroy') }}",
